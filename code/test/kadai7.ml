...
let parse_let_let _ =
    ...
    let expected =
        Let ("x", Let ("y", IntLit 1, Var "y"), Let ("z", Var "x", Var "z"))
    in
...
let parse_tuple2 _ =
    ...
    assert_equal ast
      (Tuple [Add (IntLit 1, IntLit 2); Add (IntLit 2, IntLit 3)])
...
let parse_tuple_list2 _ =
    let ast = parse_repl_string "[1,2;3,4]" in
    assert_equal ast
      (Cons (Tuple [IntLit 1; IntLit 2], Cons (Tuple [IntLit 3; IntLit 4], Emp)))

let eval_tuple _ =
    let value = eval_string "let x = 1 in x, 1+1, let y = 3 in y, 4" in
    let expected =
        TupleVal [IntVal 1; IntVal 2; TupleVal [IntVal 3; IntVal 4]]
    in
...
let parse_cmp_and _ =
    ...
    let expected =
        And (Gret (IntLit 3, IntLit 2), Not (Not (Less (IntLit 1, IntLit 0))))
    in
    assert_equal ast expected
...
let parse_match1 _ =
    let ast =
        parse_repl_string "match x with y -> 0 | z -> match z with a -> a"
    in
    let expected =
        Match
          ( Var "x"
...
let parse_match2 _ =
    ...
    let expected =
        Match
          ( Var "x"
          , [(PVar "y", Let ("x", IntLit 1, Var "x")); (PVar "z", IntLit 1)] )
    in
    assert_equal ast expected
...
let parse_fun _ =
    ...
    let expected =
        Fun ("x", Fun ("y", Fun ("z", Add (Add (Var "x", Var "y"), Var "z"))))
    in
    assert_equal ast expected
...
let parse_letrec _ =
    ...
    assert_equal ast
      (LetRec ("x", Cons (IntLit 1, Cons (Var "x", Emp)), Var "x"))
...
let parse_if _ =
    ...
    let expected =
        If (Eq (IntLit 1, IntLit 2), IntLit 1, Let ("x", IntLit 2, Var "x"))
    in
    assert_equal ast expected
    ...
    let expected =
        Match
          ...
          , [ ( PCons
                  ( PIntLit 1
                  , PCons
                      (PBoolLit true, PCons (PVar "x", PCons (PIntLit 1, PEmp)))
                  )
              , IntLit 0 ) ] )
    in
    assert_equal ast expected
...
let is_static_scope _ =
    try eval_string "let f x = y in let rec y = 1 in f 0" |> ignore
    with Failure msg -> assert_equal msg "unbound variable: y"

let type_if1 _ =
    try
      typecheck_string "if 1 = 1 then 1 else true" |> ignore ;
      assert_failure "fail"
    with Failure msg ->
      assert_equal msg "type of then and typeof else are unmatched"
...
let type_invalid_4arith _ =
    try
      typecheck_string "1 + true" |> ignore ;
      assert_failure "fail"
    with Failure msg -> assert_equal msg "type error in Add"
