...
let emp_zam = {code= []; rstack= []; astack= []; env= []}
...
let test_clos_and_app _ =
    let zam =
        {emp_zam with code= [PushMark; Ldi 42; Closure [Access 0; Return]; App]}
    in
    assert_equal (exec zam) (Int 42)

let test_test _ =
    let zam = {emp_zam with code= [Ldb true; Test ([Ldi 42], [Ldi 2])]} in
    assert_equal (exec zam) (Int 42)
    ...
    let zam =
        { emp_zam with
          code=
            ...
            ; PushMark
            ; Ldi 2
            ; Closure [Closure [Access 0; Access 2; Add; Return]; Return]
            ; App
            ; App ] }
    in
    ...
    let zam =
        { emp_zam with
          code=
            ...
            ; Ldi 1
            ; Ldi 2
            ; Closure [Grab; Access 0; Access 2; Add; Return]
            ; App ] }
    in
    assert_equal (exec zam) (Int 3)
