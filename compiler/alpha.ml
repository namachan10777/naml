type id_t = string list * int [@@deriving show]

type tvar_t = string [@@deriving show]
...
let rec of_expr env =
    ...
    | Ast.CtorApp (id, arg) ->
        CtorApp (lookup id venv, List.map (of_expr env) arg)
    | Ast.Tuple ts -> Tuple (List.map (of_expr env) ts)
    | Ast.If (cond, e1, e2) ->
        If (of_expr env cond, of_expr env e1, of_expr env e2)
    | Ast.Fun (args, body) ->
