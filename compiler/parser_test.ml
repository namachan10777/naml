let () =
    ...
    (Ast.Add (Ast.Int 0, Ast.Paren (Ast.Add (Ast.Int 1, Ast.Int 2))));
    Test.assert_eq "4arith" (Parser.parse @@ Lex.lex "3*(200+300)/4-5" @@ Lex.initial_pos "test.ml")
    (Ast.Sub (
        Ast.Div (
            Ast.Mul (Ast.Int 3, Ast.Paren(Ast.Add (Ast.Int 200, Ast.Int 300))),
            Ast.Int 4
        ),
        Ast.Int 5
    ));
    ...
    (Ast.And (
        ...
        Ast.Not (Ast.Not (Ast.Paren (Ast.Less (Ast.Int 1, Ast.Int 0))))
    ));
    ...
    (Ast.Add (Ast.Paren(Ast.Let ("x", Ast.Int 1, Ast.Var "x")), Ast.Int 1));
    ...
    (Ast.Pipeline (Ast.Pipeline (Ast.Int 1, Ast.Var "f"), Ast.Var "g"));
    ...
    (Ast.Seq (
        Ast.Int 1,
        Ast.Seq (
            Ast.Pipeline (Ast.Int 2, Ast.Var "f"),
            Ast.Int 3
        )
    ));
